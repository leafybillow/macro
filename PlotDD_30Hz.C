void PlotDD_30Hz(int runnum){
  ...
  int comb_first[2]={1,2};
  int comb_second[2]={3,4};
  ...
  for(int isam =0; isam < 8 ; isam++){
    ...
    R->SetAlias(Form("Ab%d_%d",isam+1,2),Form("1e6*(vqwk5_%d_b1_cal +vqwk5_%d_b2_cal - vqwk5_%d_b3_cal - vqwk5_%d_b4_cal)/(vqwk5_%d_b1_cal + vqwk5_%d_b2_cal+ vqwk5_%d_b3_cal + vqwk5_%d_b4_cal)",isam,isam,isam,isam,isam,isam,isam,isam));
    R->SetAlias(Form("Ab%d_%d",isam+1,3),Form("1e6*(vqwk5_%d_b1_cal- vqwk5_%d_b2_cal - vqwk5_%d_b3_cal+ vqwk5_%d_b4_cal)/(vqwk5_%d_b1_cal + vqwk5_%d_b2_cal+ vqwk5_%d_b3_cal + vqwk5_%d_b4_cal)",isam,isam,isam,isam,isam,isam,isam,isam));
    P->SetAlias(Form("Ab%d_%d",isam+1,4),Form("asym_blumi%d",isam+1));
  }
  ...
  for(int ibcm=0; ibcm<4; ibcm++){  // ibcm = vqwk index
    ...
    R->SetAlias(Form("Aq%d_%d",ibcm+1,2),Form("1e6*(vqwk0_%d_b1_cal +vqwk0_%d_b2_cal - vqwk0_%d_b3_cal - vqwk0_%d_b4_cal)/(vqwk0_%d_b1_cal + vqwk0_%d_b2_cal+ vqwk0_%d_b3_cal + vqwk0_%d_b4_cal)",ibcm,ibcm,ibcm,ibcm,ibcm,ibcm,ibcm,ibcm));
    R->SetAlias(Form("Aq%d_%d",ibcm+1,3),Form("1e6*(vqwk0_%d_b1_cal -vqwk0_%d_b2_cal - vqwk0_%d_b3_cal + vqwk0_%d_b4_cal)/(vqwk0_%d_b1_cal + vqwk0_%d_b2_cal+ vqwk0_%d_b3_cal + vqwk0_%d_b4_cal)",ibcm,ibcm,ibcm,ibcm,ibcm,ibcm,ibcm,ibcm));
    P->SetAlias(Form("Aq%d_%d",ibcm+1,4),Form("asym_bcm%d",ibcm+1));
  }
  ...
  // TString cut_raw="(ev_num > 10e3 && ev_num<14e3)||(ev_num>22e3 && ev_num< 28e3)||(ev_num > 32e3 && ev_num<40e3)||(ev_num>44e3 && ev_num< 50e3)";
  ...
  cut_text[4] = cut_parity;
  
  TCanvas *c_all = new TCanvas("c_all","c_all",1200,600);
  TH2D *h[5];
  c_all ->Divide(4,2);
  for(int icomb =0; icomb<5; icomb++){
  ...
  // blumi 37 48 26 15 vs bcm1,2,3,4
  for( int ibcm = 0 ;ibcm < 4; ibcm ++){  // ibcm =1  =>  to skip digital upstream bcm
	...
	TString histo_name = Form("h1colz%d%d%d",ibcm,iplot,icomb);
	...
	histo_name = Form("h11%d%d%d",ibcm,iplot,icomb);
	tree[icomb]->Draw(Form("%s >>%s",tree_name.Data(),histo_name.Data()),cut_text[icomb],"goff");
	...
	histo_name = Form("h12%d%d%d",ibcm,iplot,icomb);
	tree[icomb]->Draw(Form("%s>> %s",tree_name.Data(),histo_name.Data()),cut_text[icomb],"goff");
	...
	histo_name = Form("h13%d%d%d",ibcm,iplot,icomb);
	tree[icomb]->Draw(Form("%s >> %s",tree_name.Data(),histo_name.Data()),cut_text[icomb],"goff");
